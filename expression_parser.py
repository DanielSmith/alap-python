    ...
    def _expand_macros(self, expr: str, anchor_id: str | None) -> str:
        ...
        for _ in range(MAX_MACRO_EXPANSIONS):
            ...
            def replacer(m: re.Match[str]) -> str:
                ...
                if not name:
                    warnings.warn(
                        'Bare "@" is no longer supported — use "@macroname" '
                        "to reference a named macro in config.macros"
                    )
                    return ""
                macros = self._config.get("macros")
                if not macros or name not in macros:
                    warnings.warn(f'Macro "@{name}" not found in config.macros')
                    return ""
                macro = macros[name]
